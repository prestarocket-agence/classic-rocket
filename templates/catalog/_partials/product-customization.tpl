    ...
    <div class="card card-block">
      ...
      {block name='product_customization_form'}
        <form method="post" action="{$product.url}" enctype="multipart/form-data">
          ...
          <label for="field-{$field.id_customization_field}"> {$field.label}</label>
                {if $field.type == 'text'}
                  <textarea placeholder="{l s='Your message here' d='Shop.Forms.Help'}" class="product-message form-control" maxlength="250" {if $field.required} required {/if} name="{$field.input_name}" id="field-{$field.id_customization_field}"></textarea>
                ...
                {elseif $field.type == 'image'}
                  {if $field.is_customized}
                    ...
                    <a class="remove-image btn btn-sm btn-outline-danger" href="{$field.remove_image_url}" rel="nofollow">{l s='Remove Image' d='Shop.Theme.Actions'}</a>
                    </div>
                  {/if}
                  <div class="custom-file">
                    <input class="custom-file-input" {if $field.required} required {/if} type="file" name="{$field.input_name}" id="field-{$field.id_customization_field}">
