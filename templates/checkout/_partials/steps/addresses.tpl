...
{block name='step_content'}
    <div class="/js js-address-form">
        <form
                ...
                {if $show_delivery_address_form || ($show_invoice_address_form && !$use_same_address)}class="/js needs-validation"
                autocomplete="false" novalidate{/if}
        >
            {if !$use_same_address}
                <p class="u-h4 u-mb-2">{l s='Shipping Address' d='Shop.Theme.Checkout'}</p>
            {/if}
            ...
            {if $show_delivery_address_form}
                <div id="delivery-address" class="/js form-wrapper">
...
{*                        {l s='Ajouter une nouvelle adresse' d='Shop.Theme.Actions'}*}
                ...
                {else}
                    <p class="alert alert-danger js-address-error" name="alert-delivery"
                       style="display: none">{l s="Your address is incomplete, please update it." d="Shop.Notifications.Error"}</p>
                {/if}
